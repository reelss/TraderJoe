...
- 2026-09-04: **Joe was dead all trading day — 7 consecutive cycles failed on "credit balance is too low."** Raheel topped up mid-evening; a forced cycle then ran clean. **The runway warning I built failed completely** (reported warn=False, 857 days remaining, $12.69 tracked vs a real $0). Structural flaw: credit runs out -> calls fail -> failed calls log no usage -> measured burn -> 0 -> projected runway -> infinity. Blind exactly when it matters. Fixed: `billing.credit_exhausted()` treats an API refusal in joe.log as GROUND TRUTH overriding the estimate (self-clears once a billed call succeeds after the refusal); `days_remaining` withheld + `low_confidence` flag when <5 calls back the burn rate; audit reports exhaustion CRITICAL. **The daily audit DID catch the outage via its FATAL-error check — the safety net held even though the predictor didn't.** Note the checkpoint is now stale (set $20 on 7/31); re-run `billing --set` after any top-up.
- 2026-09-04: **The 7/31 fixes are working — measured over 5 weeks.** Before 7/31: 54 trades, 61% win, +0.40% avg, W/L **0.88x**. Since: 15 trades, 67% win, **+1.44%** avg, avg win +2.36% -> **+3.54%**, W/L **1.28x**. Critically, **3 of 15 winners reached the +3.5% ladder zone vs 0 of 54 before** — the recalibration made the ladder reachable. Ladder still shows no `trailing_stop_*` exits, but that is arguably correct: the ladder is a protective floor that only fires on a giveback, and no winner has given back its gains. The audit's "ladder has not fired" check may be too pessimistic as written.
- 2026-09-04: **Anti-ratchet system produced its first loosening signal:** blocker scoreboard now flags `below_sma` as "rule_costing_money" — 34 missed gains vs 12 correct passes over 103 resolutions. This is the learning loop working in the direction it was built for (rules can loosen, not just tighten). NOT yet acted on — needs a decision.
- 2026-09-04: Fixed stop-sync noise: a position with a pending SELL has its shares reserved, so `place_stop` returned "insufficient qty available" for every liquidating position and the audit flagged them "unprotected". Both paths now skip symbols with pending sells.
