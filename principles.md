...
- Volume is a CONVICTION SIZER, not a binary gate: vol ratio ≥ 1.5× = full size;
  ...
  **The JPM entry on 2026-08-04 is a fourth candidate violation under review: reasoning said "Cannot enter. PASS" at 0.26× vol but a buy order was submitted. Decision/execution mismatch must be resolved — if vol was sub-0.8× at submission, exit at next opportunity.**
  **The NVDA entry on 2026-08-05 is a fifth documented violation: vol was 0.45× at order submission; reasoning incorrectly stated this was above the 0.8× floor. The position must be exited at next opportunity. Factual errors about vol levels in reasoning are not an excuse — the vol ratio must be verified numerically before submission.**
  **The NVDA re-entry on 2026-08-06 (16:36 cycle) is a sixth documented violation: vol was 0.51× at submission. Reasoning stated "0.51× is in the 0.8–1.5× graduated zone" which is factually incorrect — 0.51× is below 0.8× and falls in the hard-floor no-entry zone. After correctly exiting a vol violation, re-entering the same name in the same session without vol clearing 0.8× is the identical error. Exit at next opportunity.**
  **The JPM re-entry on 2026-08-06 (14:36 cycle) is a seventh documented violation: JPM was exited for vol violation earlier the same session, then re-entered while vol remained sub-0.8×. Exit at next opportunity. Pattern note: re-entering a name immediately after a vol-violation exit, without confirming vol ≥ 0.8× at the new submission time, is a recurring failure mode.**
  **Vol-gate scoreboard (updated 2026-08-07):** `vol_gate` shows 16 missed_gain vs 51 correct_pass over 157 passes, avg_return −1.06%, net_verdict = "rule_saving_money." The <0.8× hard floor is strongly validated. No loosening warranted.
...
- **Sector SMA200 veto is absolute and cannot be overridden by any individual signal — not vol confirmation, not RSI, not MACD, not momentum, not news catalyst. If the sector ETF is below its 200-day SMA, no new longs in that sector, period.**
  ...
  *Scoreboard note (updated 2026-08-07): `sector_veto` shows 155 missed_gain vs 191 correct_pass over 718 passes, avg_return +0.67%, net_verdict "inconclusive." The missed_gain count is growing (particularly XLC/XLY names like GOOG, AMZN, MSFT). The two-close protocol is the appropriate loosening mechanism already in place. Monitor: if missed_gain continues to dominate over 2–3 more weeks, revisit the two-close vs one-close threshold.*

## Position management
- Trim 50% of a position in ONE order when RSI closes above 75. Do not submit duplicate sell orders on the same RSI signal — verify prior trim execution before placing a second sell. **The THC duplicate-sell incident (2026-07-31: three sell orders for qty 2, 1, 1 submitted without execution confirmation) and the RTX duplicate-sell incident (2026-08-03: sell orders submitted at 13:36, 14:36, 15:36, 16:36, 17:36, 18:36, and 19:36 without confirming prior fill) are both documented violations of this rule. This is a recurring execution failure — check open qty BEFORE submitting any trim order.**
...
- **After exiting a position for a vol violation, do NOT re-enter the same name in the same session without first confirming vol ≥ 0.8× at the new order-submission time. Same-session re-entries on sub-0.8× vol are the same violation as the original entry.**
